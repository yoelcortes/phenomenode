...
def create_system_haber_bosch_process(alg='sequential modular', ideal=False):
    chemicals = bst.Chemicals(['N2', 'H2', 'NH3'], cache=True)
    if ideal:
        bst.settings.set_thermo(chemicals, ideal=ideal)
    else:
    ...
    with bst.System(algorithm=alg) as sys:
        ...
        preheater = bst.SinglePhaseStage(
            'preheater',
            ins=[feed, recycle], 
            T=450 + 273.15, 
            P=P_rxn
