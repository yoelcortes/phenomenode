...
__all__ = (
    ...
    'create_tracked_system',
    'variable_convergence_table',
...
def parse_index(index, dct):
    index, chemical = index
    index, variable = index.rsplit('.', 1)
    if variable == 'Phi': 
        ...
        variable = 'Partition coefficient'
        units = '-'
    elif variable == 'S':
    ...
    if '.outs[' in index or '.ins[' in index:
        stage, _ = index.rsplit('.', 1)
        stream = eval(index, dct)
        obj = eval(stage, dct)
        if stream in obj.ins:
            stage = None
            index = '-'
            stream = stream.ID.replace('_', ' ').capitalize() + ' stream'
        elif hasattr(obj, 'phases') and stream in obj.outs:
            top, bottom = obj.phases
            ind = obj.outs.index(stream)
            if ind == 0:
                if top == 'g':
                    stream = 'Vapor'
                elif top == 'L':
                    ...
                    raise Exception('phases do not match')
            elif ind == 1:
                if top == 'g':
                    stream = 'Liquid'
                elif top == 'L':
                ...
                if top =='g':
                    if stream.phase == 'g':
                        stream = 'Vapor sidedraw'
                    else:
                        stream = 'Liquid sidedraw'
                elif top == 'L':
                    if stream.phase == 'L':
                        stream = 'Extract sidedraw'
                    else:
                        stream = 'Raffinate sidedraw'
                else:
                    raise Exception('phases do not match')
            else:
                raise Exception(f"index {ind!r} not implemented")
            index = stage
        elif stream in obj.outs:
            if len(obj.outs) == 1:
                stream = 'Effluent'
            elif isinstance(obj, bst.Distillation):
                ind = obj.outs.index(stream)
                if ind == 0:
                    stream = 'Distillate'
                elif ind == 1:
                    stream = 'Bottoms product'
                else:
                    raise Exception(f'unexpected index {ind!r} for distillation product')
            index = stage
        else:
            breakpoint()
    else:
        stream = None
    if '.stages[' in index:
        unit, stage = index.split('.stages[')
        stage = int(stage[:-1]) + 1
    else:
        stage = '-'
        unit = index
    unit = unit.replace('_', ' ').capitalize()
    chemical = split_camel(chemical)
    if stream:
        variable = variable.lower()
        variable = f'{stream} {variable}'
    else:
...
def variable_convergence_table(name, alg='sequential modular'):
    ...
    sys = create_tracked_system(name, 'sequential modular') 
    dct = sys.flowsheet.to_dict()
    profiles.columns = pd.MultiIndex.from_tuples(
        [parse_index(i, dct) for i in profiles.columns],
        names=['Unit operation', 'Stage #', 'Variable', 'Chemical', 'Units'],
    )
...
def get_specifications(unit):
    specifications = []
    unit_name = unit.ID.replace('_', ' ').capitalize()
    def add(value, variable, chemical='-', units=None, n=None):
        vlow = variable.lower()
        if 'heat' in vlow: units = 'kJ/hr'
        elif 'temperature' in vlow: units = 'K'
        elif 'flow rate' in vlow: units = 'kmol/hr'
        elif 'pressure' in vlow: units = 'Pa'
        else: units = '-'
        if n is None:
            n = '-'
        else:
        ...
        specifications.append(
            ((unit_name, n, variable, chemical, units), value)
        )
    
    if isinstance(unit, bst.MultiStageEquilibrium):
        if unit.phases == ('g', 'l'):
            for n, stage in enumerate(unit.stages):
                if stage.T_specification is not None:
                    add(stage.T, 'Temperature', n=n)
                elif stage.B_specification is not None:
                    add(stage.B, 'Phase ratio', n=n)
                elif stage.Q is not None:
                    add(stage.Q, 'Heat', n=n)
                if stage.top_split:
                    add(stage.top_split, 'Vapor side draw fraction', n=n)
                if stage.bottom_split:
                    add(stage.bottom_split, 'Liquid side draw fraction', n=n)
        elif unit.phases == ('L', 'l'):
            if unit.partition_data:
                partition_data = unit.partition_data
                IDs = partition_data['IDs']
                Ks = partition_data['K']
                for n, K in enumerate(Ks):
                    for ID, k in zip(IDs, K):
                        ID = split_camel(ID)
                        add(k, 'Partition coefficient', ID, n=n)
            for n in range(unit.N_stages):
                add(0, 'Heat', n=n)
        else:
            raise NotImplementedError
    elif isinstance(unit, bst.Distillation):
        LK, HK = [split_camel(i) for i in unit.LHK]
        if unit.product_specification_format == 'Recovery':
            add(unit.Lr, 'Light key recovery', LK)
            add(unit.Hr, 'Heavy key recovery', HK)
        elif unit.product_specification_format == 'Composition':
            add(unit.y_top, 'Distillate light key mol fraction', LK)
            add(1 - unit.x_bot, 'Bottoms product heavy key mol fraction', HK)
        else:
            raise RuntimeError(f"unexpected product specification format {unit.product_specification_format!r}")
    elif isinstance(unit, bst.StageEquilibrium):
        if unit.phases == ('g', 'l'):
            if unit.T_specification is not None:
                add(unit.T, 'Temperature')
            elif unit.B_specification is not None:
                add(unit.B, 'Phase ratio')
            elif unit.Q is not None:
                add(unit.Q, 'Heat')
            if unit.top_split:
                add(unit.top_split, 'Vapor side draw fraction')
            if unit.bottom_split:
                add(unit.top_split, 'Liquid side draw fraction')
        elif unit.phases == ('L', 'l'):
            if unit.partition.partition_data:
                partition_data = unit.partition.partition_data
                IDs = partition_data['IDs']
                Ks = partition_data['K']
                for ID, K in zip(IDs, Ks):
                    ID = split_camel(ID)
                    add(K, 'Partition coefficient', ID)
            add(0, 'Heat')
            if unit.top_split:
                add(unit.top_split, 'Extract side draw fraction')
            if unit.bottom_split:
                add(unit.top_split, 'Raffinate side draw fraction')
        else:
            raise NotImplementedError
    elif isinstance(unit, bst.SinglePhaseStage):
        if unit.T is not None:
            add(unit.T, 'Temperature')
        elif unit.Q is not None:
            add(unit.Q, 'Heat')
    elif isinstance(unit, bst.ReactivePhaseStage):
        if unit.T is not None:
            add(unit.T, 'Temperature')
        elif unit.Q is not None:
            add(unit.Q, 'Heat')
        ...
        add(unit.split[0], 'Splitting fraction')
    add(unit.outs[0].P, 'Pressure')
...
def specifications_table(name):
    ...
    return pd.DataFrame(
        values,
        index=pd.MultiIndex.from_tuples(index, names=('Unit operation', 'Stage #', 'Variable', 'Chemical', 'Units')),
        columns=['Value'],
    ).sort_index()

def save_all_system_reports():
...
def system_report(name):
    ...
    for i in sys.units:
        i.ID = i.ID.capitalize().replace('_', ' ')
        i.line = ''
    try:
        with bst.preferences.temporary() as p:
            p.reset()
            p.light_mode()
            sys.diagram(file='flowsheet', dpi='300', format='png')
    except:
    ...
    for alg in algorithms:
        bst.report.tables_to_excel(
            [variable_convergence_table(name, alg)], 
            writer, alg.capitalize(),
        )
    writer.close()
...
def get_variable_profiles(
    ...
    if not mock and load:
        try:
            ...
            return get_variable_profiles(
                name, algorithm, number, False, mock, convergence_rate,
                grouped_error, error
            )
    else:
        if mock:
